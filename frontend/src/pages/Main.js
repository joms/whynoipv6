...
const Main = () => {
    const { sites, isFetching, fetchList } = useContext(ListContext);

    useEffect(() => {
        if (!sites.length && !isFetching) {
            fetchList();
        }
    });
    ...
    return (
        <React.Fragment>
            <p>
                ...
                specifications. We have come a long way since then, surpassing milestones like allocating the last, big
                IPV4 address blocks in 2011 to IEFT declaring IPv6 as an Internet Standard in 2017. Sadly, only a
                fraction of the top one million websites today is IPv6-compliant.
            </p>
            <p>
                So today we need to start shaming. <strong>Shame! Shame! Shame!</strong>
            ...
            <p>
                As a part of this shaming-strategy, we supply all interested parties with an updated list over Alexa’s
                top 1 million websites and their corresponding (lack of) IPv6 support clearly stated.
                <br />
                Each of the listed websites (top 100) lack an AAAA-record. In addition the nameserver-support for IPv6
                is displayed.
            </p>
            <hr />
